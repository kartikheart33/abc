package com.alexios.android.firestore

import android.app.Activity
import android.util.Log
import com.alexios.android.activities.CategoryActivity
import com.alexios.android.activities.SplashActivity
...
class FirestoreClass {
    ...
    /**
     * A function to get Wines from from FireStore Database.
     */


    fun getFilter(activity: Activity){
        mFireStore.collection(Constants.Filter)
            .get()
            .addOnSuccessListener {document ->
                Log.i(activity.javaClass.simpleName, document.toString())
                val filter = document.toObjects(Filter::class.java)
                when (activity) {
                    is SplashActivity -> {
                        // Call a function of base activity for transferring the result to it.
                        activity.filterFromFirestore(filter)
                    }
                }
            }
    }

    fun getProducts(activity: Activity){
        mFireStore.collection(Constants.PRODUCTS)
            .get()
            .addOnSuccessListener {document ->
                Log.i(activity.javaClass.simpleName, document.toString())
                // Here we have received the document snapshot which is converted into the Wine Data model object.
                val products = document.toObjects(Products::class.java)
                when (activity) {
                    is SplashActivity -> {
                        // Call a function of base activity for transferring the result to it.
                        activity.productsFromFirestore(products)
                    }
                }
            }
    }
    fun getWines(activity: Activity) {
        // Here we pass the collection name from which we wants the data.
        mFireStore.collection(Constants.WINES)
            .get()
            .addOnSuccessListener { document ->
                Log.i(activity.javaClass.simpleName, document.toString())
                // Here we have received the document snapshot which is converted into the Wine Data model object.
                val wines = document.toObjects(Wine::class.java)!!
                when (activity) {
                    is SplashActivity -> {
                        // Call a function of base activity for transferring the result to it.
                        activity.winesFromFirestore(wines)
                    }
                }
//
            }
